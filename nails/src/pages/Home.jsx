...
import { Instagram, Sparkles, Heart, Clock, BadgeCheck, MapPin, ChevronLeft, ChevronRight } from 'lucide-react'
...
const CARD_W   = 260  // px width of each card
const CARD_GAP = 16
...
function SnapGallery({ images, onImageClick }) {
  ...
  /* sync active when user swipes manually */
  const onScroll = () => {
    const el = trackRef.current
    if (!el) return
    const idx = Math.round(el.scrollLeft / (CARD_W + CARD_GAP))
    const max = getMaxActive()
    setActive(Math.max(0, Math.min(idx, max)))
  }

  const btnStyle = (disabled) => ({
    width: 40, height: 40, borderRadius: '50%',
    border: '1px solid rgba(200,160,174,0.5)',
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      >
        ...
        <button onClick={next} disabled={active >= getMaxActive()} style={btnStyle(active >= getMaxActive())}>
  ...
  return (
    ...
    >
      ...
      <section style={{ paddingBottom: '1rem', paddingTop: '3rem', overflow: 'hidden' }}>
        ...
        {/* ── Snap Gallery ── */}
        <SnapGallery images={ALL_WORKS} onImageClick={(src) => { setSelectedImage(src); setIsModalOpen(true); }} />
      </section>
